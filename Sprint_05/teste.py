...
import re 
from Connection import criar_tabela_cadastro_pessoa_fisica, inserir_dados_cadastro_pessoa_fisica, criar_tabela_bike, inserir_dados_bike, tabela_existe, select_cadastro, select_bike
...
def acessorios(escolha_acessorio):
    try: 
        acessorio = {"Acessórios": [], "Preço": 0.0}  # Inicialize a variável acessorio com valores padrão
        ...
        if escolha_acessorio == 1:
            ...
            while True:
                acessorio_nome = input("Informe o nome do acessório (ou 'fim' para encerrar): ")
                if acessorio_nome.lower() == 'fim':
                    break

                preco_str = input(f"Informe o preço do acessório '{acessorio_nome}': ").replace(',', '.')
                preco = float(preco_str)

                lista_acessorio.append({"Acessório": acessorio_nome, "Preço": preco})
                total_preco += preco

            acessorio["Acessórios"] = lista_acessorio
            acessorio["Preço"] = total_preco

            print(f"Total do preço dos acessórios: {total_preco}")
        elif escolha_acessorio == 2:
            print("Nenhum acessório adicionado.")
    except ValueError:
        print ("Valor de entrada incorreta. Tente novamente.")
    return acessorio
        ...
        match escolha_correcao:
            case 1:
                ...
                while True:
                    ...
                    if escolha_alteracao == 7:
                        break  # Sai do loop se a opção for 7 (Sair).
                    elif escolha_alteracao not in range(1, 7):
                        print("Opção inválida. Tente novamente.")
                    else:
                        opcoes = ["Nome", "CPF", "Endereço", "Telefone", "E-mail", "Data de Nascimento"]
                        dados_cadastro[opcoes[escolha_alteracao - 1]] = input(f"Nova {opcoes[escolha_alteracao - 1]}: ")
            case 2:
